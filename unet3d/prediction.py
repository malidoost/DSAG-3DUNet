...
def patch_wise_prediction(model, data, overlap=0, batch_size=1, permute=False):
	...
	output_shape = [int(model.output[3].shape[1])] + list(data.shape[-3:])
...
def predict_from_data_file(model, open_data_file, index):
	_,_,_,pred = model.predict(open_data_file.root.data[index])
	return pred 


def predict_and_get_image(model, data, affine):
	_,_,_,pred = model.predict(data)
	return nib.Nifti1Image(pred[0, 0], affine)
...
def predict_with_permutations(model, data):
	...
	for permutation_key in generate_permutation_keys():
		...
		_,_,_,pred = model.predict(temp_data)
		predictions.append(reverse_permute_data(pred[0], permutation_key))
